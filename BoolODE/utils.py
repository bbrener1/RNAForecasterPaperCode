...
def generateInputFiles(resultDF, splicedDF, outputfilenames, BoolDF, withoutRules,
                       parameterInputsPath,
                       outPrefix=''):
    """
    ...
    for f in outputfilenames:
        ...
        if len(resultDF.columns) < 1e5:
            ...
            resultDF.to_csv(outPrefix+'UnsplicedData.csv',sep=',')
            
            columns = list(splicedDF.columns)
            columns = [c.replace('-','_') for c in columns]
            splicedDF.columns = columns
            if len(parameterInputsPath) == 0:
                splicedDF = splicedDF.drop(withoutRules, axis=0)
            splicedDF.to_csv(outPrefix+'splicedData.csv',sep=',')
        else:
...
def sampleCellFromTraj(cellid,
    ...
    if writeProtein:
        ...
        for ri in varmapper.keys():
            ...
            sampleDict2[varmapper[ri]] = {header[cellid]: P[ri][timepoint-1]}
    else:
        # Default, only mRNA
        if len(proteinlist) == 0:
            for ri in rnaIndex:
    ...
    sampleDF = pd.DataFrame(sampleDict)
    sampleDF2 = pd.DataFrame(sampleDict2)
    return sampleDF, sampleDF2
