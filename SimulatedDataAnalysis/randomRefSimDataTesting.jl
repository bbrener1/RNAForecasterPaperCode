...
for k=1:117
    ...
    try
        ...
        stabilityThreshold = 2.5f0*maximum(t1[:,:,k]), stabilityChecksBeforeFail = 10)

        trainLosses[:,k] = trainingResults[2][:,1]
        validationLosses[:,k] = trainingResults[2][:,2]

        save_object("Networks/NeuralODEExpressionForecaster_RandRef" * string(k+2) * ".jld2", trainingResults[1])
...
#save data
writedlm("Losses/SimDataRandRefTrainLosses.csv", trainLosses, ',')
writedlm("Losses/SimDataRandRefValidationLosses.csv", validationLosses, ',')
writedlm("Losses/SimDataPredictionErrorSimTime.csv", predictionErrors, ',')
